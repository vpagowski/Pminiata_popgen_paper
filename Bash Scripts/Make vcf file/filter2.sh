...
for pop in $(awk '{print $6}' ${INPUT}.fam | sort | uniq); do   awk -v p="$pop" '$6==p{print $1, $2}' ${INPUT}.fam > ${pop}.keep;  plink --bfile ${INPUT} --keep ${pop}.keep --make-bed --out ${INPUT}_${pop} --allow-extra-chr --double-id;   plink --bfile ${INPUT}_${pop} --geno 0.5 --make-bed --out $INPUT_${pop}.filtered --allow-extra-chr --double-id --biallelic-only; done
...
for pop in $(awk '{print $6}' ${INPUT}.fam | sort | uniq); do
   ...
   vcftools --gzvcf $INPUT_VCF_after_filters  \
            ...
            --recode \
            --recode-INFO-all \
            --out ${INPUT}_${pop}.filtered
done

#Then, bgzip and tabix each file before merging with plink or bcftools
plink --bfile ${INPUT}_${pop1}.filtered --merge-list merge_list.txt --make-bed --out merged_all --allow-extra-chr --double-id --biallelic-only
